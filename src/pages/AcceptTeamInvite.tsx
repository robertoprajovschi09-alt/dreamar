...
export default function AcceptTeamInvite() {
  ...
  useEffect(() => {
    (async () => {
      ...
      supabase.rpc("mark_team_invite_opened", { _token: token }).then(() => {});
      const { data, error } = await supabase.rpc("get_team_invite_preview", { _token: token });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await refresh();
      toast.success(`Bine ai venit în ${preview.agency_name}!`);
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signUp({
      email, password,
      options: {
        emailRedirectTo: `${window.location.origin}/accept-team-invite?token=${token}`,
        data: { full_name: fullName, team_invite_token: token },
      },
    });
    setBusy(false);
    if (error) toast.error(error.message);
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center mb-6">
              <div className="text-xs uppercase tracking-widest text-accent font-semibold mb-1">Invitație în echipă</div>
              <h1 className="text-xl font-bold">Alătură-te echipei {preview.agency_name}</h1>
              <p className="text-sm text-muted-foreground mt-1">Rol: <span className="text-foreground">{preview.role === "agency_owner" ? "Owner" : "Membru echipă"}</span></p>
            </div>
