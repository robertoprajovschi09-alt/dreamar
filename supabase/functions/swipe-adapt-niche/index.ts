// AI: adapt a swipe to another niche.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { swipe_id, target_niche } = await req.json();
    if (!swipe_id || !target_niche) return j({ error: "swipe_id and target_niche required" }, 400);
    ...
    const sys = `You adapt successful marketing ideas across niches. Rewrite the hook and script for the target niche while keeping the original psychological angle and structure. Be specific to the target niche audience. Never invent statistics or claims.`;
    ...
    const aiRes = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `Original (niche: ${sw.niche || "unspecified"}):\n${JSON.stringify({ title: sw.title, hook: sw.hook, script: sw.script, caption: sw.caption, content_angle: sw.content_angle, type: sw.type }, null, 2)}\n\nTarget niche: ${target_niche}` },
        ],
        tools: [{
          type: "function",
          function: {
            ...
            parameters: {
              ...
              required: ["title", "hook", "script", "caption", "suggested_tags"],
    ...
    console.error("swipe-adapt-niche:", e);
